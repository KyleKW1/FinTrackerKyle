...
def load_all_user_data(user_id):
    ...
    try:
        ...
        return pd.DataFrame()
# ============================================
# ENHANCED PDF PROCESSING - Add these functions to your Test3.py
# Replace your existing extract_from_pdf() function
# ============================================
...
def parse_ncb_transaction_line(line, year):
    """
    Parse a single NCB transaction line.
    NCB Format: DD/Mon DESCRIPTION AMOUNT BALANCE
    Example: "02/Sep ELink TRF-FR RICK EVERTON DARBY 4,000.00 4,004.23"
    ...
    try:
        ...
        with pdfplumber.open(pdf_file) as pdf:
            for page_num, page in enumerate(pdf.pages, 1):
                ...
                if page_num == 1:
                    # First priority: Look for statement end date pattern like "PRATVILLE P.O., 30-09-2024"
                    # This is the statement period end date
                    statement_date_match = re.search(r'P\.O\.,\s*(\d{2})-(\d{2})-(\d{4})', text)
                    if statement_date_match:
                        year = statement_date_match.group(3)
                    else:
                        # Fallback: Find all 4-digit years and use the maximum
                        all_years = re.findall(r'\b(20\d{2})\b', text)
                        if all_years:
                            year_ints = [int(y) for y in all_years]
                            year = str(max(year_ints))
                        else:
...
def extract_from_pdf(pdf_file):
    ...
    if match:
        try:
            ...
            # balance_str = match.group(4)  # We don't need balance for now
            
            # Convert date format: DD/Mon to DD/Mon/YYYY
            full_date = f"{date_str}/{year}"
            
            # Convert amount to float
            amount = float(amount_str)
            
            # Determine category based on sign
...
,
        ...
        # Show debug info
        if debug and all_lines:
            with st.expander("🔍 DEBUG: Sample PDF Lines (First 30 lines)", expanded=True):
                st.write(f"**Detected Year:** {year}")
                st.write("**Sample lines from PDF:**")
                for i, line in enumerate(all_lines[:30], 1):
                    if line.strip():
                        # Show if line would be parsed
                        parsed = parse_ncb_transaction_line(line, year)
                        if parsed:
                            st.code(f"✅ {i}: {line}", language=None)
                        else:
