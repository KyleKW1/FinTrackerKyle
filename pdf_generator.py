...
def create_comprehensive_pdf(data, selected_year, selected_months, analysis_type, user_id):
    ...
    period_data = data[(data['Year'] == selected_year) & (data['Month'].isin(selected_months))].copy()
    
    if period_data.empty:
        raise ValueError("No data available for selected period")
    
    # Convert all numeric columns to float to avoid Decimal issues
    if 'Amount' in period_data.columns:
        period_data['Amount'] = period_data['Amount'].apply(convert_to_float)
    ...
    # Calculate total stats - convert to float
    total_income = 0.0
    total_spending = 0.0
    ...
    for month_num in selected_months:
        ...
        total_income += convert_to_float(stats['income'])
        total_spending += convert_to_float(stats['spending'])
    ...
    SAVINGS_GOAL = convert_to_float(prefs.get('savings_goal', 5000) if prefs else 5000)
    ...
    try:
        ...
        for month_num in sorted(selected_months):
            ...
            months_data.append({
                ...
                'Income': convert_to_float(stats['income']),
                'Spending': convert_to_float(stats['spending']),
                'Savings': convert_to_float(stats['savings'])
            })
        ...
        for month_num in selected_months:
            ...
            if not month_summary.empty:
                ...
                month_summary['Amount'] = month_summary['Amount'].apply(convert_to_float)
        ...
        if all_spending:
            ...
            aggregate_summary['Amount'] = aggregate_summary['Amount'].apply(convert_to_float)
            ...
            for idx, row in aggregate_summary.iterrows():
                ...
                amount_val = convert_to_float(row['Amount'])
                pct_val = convert_to_float(row['Percentage'])
                
                pdf.cell(90, 8, str(row['Spending Category']), 1, 0, 'L', True)
                pdf.cell(50, 8, f"J${amount_val:,.2f}", 1, 0, 'R', True)
                pdf.cell(40, 8, f"{pct_val:.1f}%", 1, 1, 'R', True)
            ...
            pdf.set_fill_color(102, 126, 234)
            pdf.set_text_color(255, 255, 255)
            total_amount = convert_to_float(aggregate_summary['Amount'].sum())
            pdf.cell(90, 8, 'TOTAL', 1, 0, 'L', True)
            pdf.cell(50, 8, f"J${total_amount:,.2f}", 1, 0, 'R', True)
        ...
        for idx, row in top_transactions.iterrows():
            ...
            pdf.cell(35, 7, f"J${amount_val:,.2f}", 1, 0, 'R', True)
